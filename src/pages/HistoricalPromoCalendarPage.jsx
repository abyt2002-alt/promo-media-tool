...
const renderSummaryBullet = (bullet) => {
  const text = String(bullet || '').trim()
  const sepIndex = text.indexOf(':')
  if (sepIndex <= 0 || sepIndex >= text.length - 1) return text
  const lead = text.slice(0, sepIndex).trim()
  const tail = text.slice(sepIndex + 1).trim()
  return (
    <>
      <span className="font-semibold text-slate-800">{lead}:</span> {tail}
    </>
  )
}
                ...
                <>
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-5">
                    {summary.bullets.map((bullet, idx) => (
                      <li key={`summary-${idx}`}>{renderSummaryBullet(bullet)}</li>
                    ))}
                  </ul>
                  <p className="mt-2 text-[11px] font-medium text-slate-500">
                    Source: {summary.source === 'gemini' ? 'Trinity AI' : 'Trinity AI (Fallback)'}
                  </p>
                </>
              ) : (
